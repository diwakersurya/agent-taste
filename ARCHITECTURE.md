...
| `src/vault.ts` | Vault location (`~/.agent-taste` symlink), `config.json`, atomic writes, file lock, log. |
...
1. `init` creates the vault, links `~/.agent-taste`, copies the CLI bundle to `<vault>/bin/`, and adds marked blocks / entries to each chosen tool's config.
...
- Vault: `taste.md` (source of truth), `taste.json` (generated), `config.json` (settings; the remote token only as a SHA-256 hash), `capture.log`, `bin/agent-taste.js`.
- Tool configs: content between `<!-- agent-taste:start -->` / `<!-- agent-taste:end -->`, a `SessionEnd` entry whose command contains `agent-taste.js`, and an `mcpServers["agent-taste"]` entry. The first time a user file is changed it is copied to `<file>.bak-agent-taste`.
