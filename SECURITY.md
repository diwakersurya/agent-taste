...
`taste-profile mcp --http` is the only network-facing part.
...
- Requires a 256-bit random token (`taste-profile integrate chatgpt` or `mcp --rotate-token`). Only its SHA-256 is stored. Accepted as `Authorization: Bearer <token>` or in the path `/mcp/<token>`; compared in constant time.
