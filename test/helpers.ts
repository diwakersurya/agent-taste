...
export function tmpHome(): string {
  const d = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "agent-taste-")));
  process.env.HOME = d;
  return d;
}
...
export function makeVault(home: string, md = MIN): string {
  const v = path.join(home, "My Drive", "agent-taste");
  fs.mkdirSync(v, { recursive: true });
  fs.writeFileSync(path.join(v, "taste.md"), md);
  fs.symlinkSync(v, path.join(home, ".agent-taste"));
  return v;
}
